...
function executeGitHubCLI(prompt, instructId) {
  ...
  try {
    // Primary: GitHub Copilot CLI in non-interactive agent mode
    // Uses -p for prompt, -s for silent (script-friendly output),
    // --allow-all-tools + --no-ask-user for autonomous execution
    ...
    success = true;
    log('INFO', `GitHub Copilot CLI (agent mode) completed for INST-${instructId}`);
  } catch (e1) {
    log('WARN', `gh copilot agent mode failed: ${(e1.message || '').substring(0, 300)}`);
    try {
      // Fallback: gh copilot suggest with correct model name
      output = execSync(
        `cd "${WORKSPACE}" && gh copilot suggest --model claude-opus-4.6 -t shell < "${promptFile}"`,
        { encoding: 'utf8', timeout: 300000, maxBuffer: 10 * 1024 * 1024 }
      );
      success = true;
      log('INFO', `gh copilot suggest completed for INST-${instructId}`);
    } catch (e2) {
      log('WARN', `gh copilot suggest failed: ${(e2.message || '').substring(0, 300)}`);
      try {
        // Fallback 2: gh copilot explain with correct model name
        output = execSync(
          `cd "${WORKSPACE}" && gh copilot explain --model claude-opus-4.6 < "${promptFile}"`,
          { encoding: 'utf8', timeout: 300000, maxBuffer: 10 * 1024 * 1024 }
        );
        success = true;
      } catch (e3) {
        output = `CLI execution failed. Tried:\n1. gh copilot -p (agent mode): ${(e1.message || '').substring(0, 300)}\n2. gh copilot suggest: ${(e2.message || '').substring(0, 300)}\n3. gh copilot explain: ${(e3.message || '').substring(0, 300)}\n\nPrompt was saved to: ${promptFile}\nRun manually: gh copilot -p "<prompt>" --model claude-opus-4.6 --allow-all-tools`;
        success = false;
      }
    }
  }
  ...
  if (args.includes('--status')) {
    ...
    (async () => {
      ...
      try {
        ...
        try {
          const copilotVersion = execSync('gh copilot -- --version', { encoding: 'utf8', timeout: 10000 });
          console.log(`  ✅ gh copilot: ${copilotVersion.trim().split('\n')[0]}`);
          console.log(`  ✅ Model: claude-opus-4.6 (primary), suggest/explain (fallbacks)`);
        } catch {
          console.log(`  ⚠️ gh copilot extension not available`);
        }
      } catch {
        console.log(`  ⚠️ gh CLI not found — INSTRUCT pipeline will fail`);
      }
