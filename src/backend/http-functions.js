...
import {
    ...
    get_ec_get_ec_profile, options_ec_get_ec_profile,
    post_ec_update_ec_profile, options_ec_update_ec_profile,
    get_ec_members_profiles, options_ec_members_profiles
...
// GET /_functions/admin_portal — redirect to GitHub Pages (no custom domain)
export function get_admin_portal(request) {
    return wixResponse({ status: 302, headers: { 'Location': 'https://banfjax-hash.github.io/banf/admin-portal.html', 'Access-Control-Allow-Origin': '*', 'Cache-Control': 'no-store' }, body: '' });
}
export function options_admin_portal(request) { return handleCors(); }
export function get_admin_portal_raw(request) { return get_admin_portal(request); }
