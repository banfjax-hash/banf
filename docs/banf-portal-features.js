...
window.BANF_RAG = {
  ...
  initKnowledgeBase: function(){
    ...
    docs.push({id:'kb-qr-food-serving',year:'2026-27',category:'event',title:'QR Food-Serving Check-In System — Event Day Workflow',
      text:'QR Food-Serving System (Phase 7C). PURPOSE: Generate QR codes for RSVP-Yes attendees, scan at food counter for check-in and dietary tracking. QR PAYLOAD: Each QR code encodes JSON with name, email, adults count, kids count, veg/nonVeg servings, dietary notes, householdType, membershipTier, paid status, amount, regCode. AGENT: banf-qr-food-agent.js (Node.js) reads from banf-crm-reconciliation.json, generates QR PNG files in qr-codes/ folder, creates Excel verification sheet with columns: email, name, QR code file, decoded QR data (adults, kids, veg, non-veg, dietary), payment status, amount, membership category/tier, admin comment column. Also creates HTML report. STEP-BY-STEP PROTOCOL: 1) Generate QR codes 2) Create Excel verification 3) Create HTML report 4) Admin reviews all data 5) Admin approves email send 6) Post-send report. No auto-send — admin must explicitly approve. EC PORTAL: QR Food Serving panel in admin-portal.html under EC Admin section. Features: real-time check-in tracking, manual walk-in entry, food prep summary, CSV export, print check-in sheet. Any EC admin can access this. EVENTS: Works for any event (Bosonto Utsob 2026, Saraswati Puja 2026, etc).'});
