...
window.BANF_RAG = {
  ...
  initKnowledgeBase: function(){
    ...
    docs.push({id:'kb-email-dedup-policy',year:'general',category:'process',title:'Communication Agent Email Dedup Policy — Cross-System Guard',
      text:'BANF Communication Agent Email Guard Policy (mandatory for all outbound emails across ALL systems). ARCHITECTURE: There are 6 email-sending systems — admin-portal.html (guarded by communicationAgentSend), ec-signup-reminder-agent.js (standalone Node), ec-reminder-send.js (standalone Node), ec-onboarding-gate.js (Wix backend), ec-onboard-dashboard.html (Wix frontend), membership-drive.js (Wix backend). ALL systems now enforce dedup guards. SHARED STATE: ec-onboard-status.json is the single source of truth for EC member signup status. Both standalone scripts read from this file instead of using hardcoded rosters. This prevents sending reminders to members who have already completed the workflow. FILE-BASED DEDUP: Standalone scripts use ec-onboard-status.json emailLog.sent to track last-sent timestamps per recipient. Default cooldown is 7 days (168 hours). Use --force flag to override. WORKFLOW COMPLETE GATE: If ec-onboard-status.json has workflowComplete=true, ALL standalone scripts refuse to send any emails. WIX BACKEND COOLDOWN: ec-onboarding-gate.js checks lastReminderSent per member in AdminRoles collection with 72-hour cooldown between reminders. ADMIN PORTAL GUARD: communicationAgentSend() with localStorage EMAIL_SENT_LOG (90-day retention) protects the admin portal HTML. Override requires --force flag (CLI) or written reason (portal). All sends logged for full audit transparency.'});

    docs.push({id:'kb-ec-onboard-status-file',year:'general',category:'process',title:'EC Onboard Status File — Single Source of Truth',
      text:'The file ec-onboard-status.json (located at project root) is the single source of truth for EC member signup status. It contains: fiscalYear (FY2026-27), workflowComplete flag (boolean), members array with signedUp/signupDate per member, and emailLog with per-recipient sent timestamps and configurable cooldownHours (default 168 = 7 days). Both ec-signup-reminder-agent.js and ec-reminder-send.js read this file at startup. When a member completes signup, update their signedUp=true and signupDate in this file. When ALL members complete, set workflowComplete=true to stop all reminder sends. The file is automatically updated with sent timestamps after each successful email send.'});

    docs.push({id:'kb-qr-food-serving',year:'2026-27',category:'event',title:'QR Food-Serving Check-In System — Event Day Workflow',
      text:'QR Food-Serving System (Phase 7C/7D). PURPOSE: Generate QR codes for RSVP-Yes attendees, scan at food counter for check-in and dietary tracking. QR PAYLOAD: Each QR code encodes JSON with type=BANF_FOOD_CHECK_IN, regCode, name, email, adults count, kids count, veg/nonVeg servings, dietary notes, householdType, membershipTier, paid status, amount, event name. AGENT: banf-qr-food-agent.js (Node.js) reads from banf-crm-reconciliation.json, generates QR PNG files in qr-codes/ folder, creates Excel verification sheet. CAMERA SCANNER (Phase 7D): EC admin portal has a live camera-based QR scanner using jsQR library. When a QR code is scanned, it opens a full Attendee Detail Modal showing: Evite/RSVP details (event, RSVP status, adults, kids, dietary), Payment details (paid status, amount, method), Membership details (tier, category, household type, EC member, member since), Food serving status. Admin can verify confirmation, update membership recommendation, change verification status (verified/pending/mismatch/flagged), add notes, and mark as served. EXCEL VERIFICATION: Export Verification Excel button generates CSV with all attendee data including admin verification status and notes. Admin verifies in Excel before QR codes are sent to attendees. EC PORTAL: QR Food Serving panel in admin-portal.html. Clickable table rows open the detail modal for any attendee.'});

    docs.push({id:'kb-payment-pipeline',year:'2026-27',category:'finance',title:'Membership Payment Pipeline — Wells Fargo Zelle & Acknowledgment',
      text:'BANF Membership Payment Pipeline (active). OVERVIEW: 9-stage agentic pipeline processes member payments from Zelle to full membership activation. STAGES: 1) Email Scan 2) Evite Analysis 3) Wells Fargo Payment Scan 4) Membership Selection 5) Payment Match 6) Payment Collection 7) Communication Verification 8) Drive Signup 9) Event Management. WELLS FARGO SCANNER: Scans Gmail for WF Zelle notifications (from alerts@notify.wellsfargo.com), extracts amount/sender, matches to CRM members. PAYMENT MATCH: Auto-matches payment amounts to membership categories (M2 Premium EB Family $375, Couple $290, Individual $215, Student $145; M1 Regular Family $280, Couple $255, etc.). ACKNOWLEDGMENT EMAILS: 8 communication templates — rsvp_confirmation, membership_prompt, payment_received_auto, payment_instructions, payment_reminder, verification_request, drive_invitation, event_confirmation. Each goes through Communication Agent Guard. EC PORTAL: Payment Agent Status visible in Member Dues panel (admin-portal.html). Run Agent Sync button triggers batch sync. Payment Drive section shows unpaid members with Send Reminder and Mark Paid actions. This pipeline is active and must remain operational for FY2026-27.'});

    docs.push({id:'kb-email-pipeline-status',year:'2026-27',category:'process',title:'Email Processing Pipeline — Always-Active with Portal Visibility',
      text:'Email Processing Pipeline (Phase 7D). The bosonto-email-reader-agent.js runs continuously with 5-minute polling intervals. It scans Gmail for: 1) Evite RSVP notifications (new RSVPs parsed and added to pipeline), 2) Zelle/WF payment emails (matched to CRM, auto-acknowledgments sent), 3) Admin instruction emails (processed as action items), 4) User query emails (classified by RAG and auto/manual reply). ALWAYS-ON: Run with --poll --auto-send --interval=5 for continuous operation. STATUS FILE: After each poll cycle, writes banf-pipeline-status.json with full status: pipelineStatus, health, lastScanTime, nextPollAt, pollCount, totals, activeDrives, recentActivity. EC PORTAL: Email Pipeline panel in admin-portal.html shows live pipeline status — KPIs, active pipelines, last cycle metrics, recent activity feed, configuration. Auto-refreshes every 30 seconds. Pipeline routes emails to correct pipeline based on sender/content: Evite RSVPs → Member Pipeline, Zelle Payments → CRM Update + Payment Followup, Admin Instructions → Action Processor, User Queries → RAG Classification.'});

    // ── Episode Memory: CSV Ground Truth Pipeline Learnings (March 2026) ──
    docs.push({id:'kb-episode-csv-ground-truth',year:'2026-27',category:'episode_memory',title:'Episode: Evite CSV vs Email Reader Pipeline — Critical Learnings',
      text:'EPISODE MEMORY (March 6, 2026 — Bosonto Utsob investigation). PROBLEM: Email reader pipeline (bosonto-email-reader-agent.js) scanned 657 Evite RSVP notification emails and identified 47 Yes attendees. But the data was incomplete — no email addresses, no phone numbers, no food preferences, and 6 Evite usernames could not be matched to real names. SOLUTION: Downloaded the Evite admin CSV export from the banfjax@gmail.com inbox (subject: "guestlist (1)", March 6 2026). The CSV is the GROUND TRUTH with columns: Name, Email, Phone, Status, Number In Party, RSVP Comment. PIPELINE GAPS IDENTIFIED: GAP-001 [CRITICAL] Evite RSVP notification emails contain guest NAME and RSVP status only — email/phone are NOT included. GAP-002 [HIGH] Food preferences are in the CSV RSVP Comment field, not reliably in notification emails. GAP-003 [MEDIUM] Party size in CSV is authoritative — email text may have inconsistent adult/kid parsing. GAP-004 [MEDIUM] 6 Evite usernames (Chakfam5, Dindrani74, Kalyan32258, Mdhar79, Priyanka16587, Soma1979P) cannot be resolved without CSV email matching. MANDATORY PATTERN FOR FUTURE: 1) Email reader pipeline is for INCREMENTAL RSVP detection only. 2) For authoritative attendee data, always download the Evite admin CSV export. 3) Use CSV email field as primary matching key (not name). 4) Payment matching must use STRICT email-based matching only — NEVER use single-name fuzzy matching (last name alone caused false matches with Ghosh, Saha, Mondal families).'});

    docs.push({id:'kb-episode-payment-matching',year:'2026-27',category:'episode_memory',title:'Episode: Payment Matching Must Be Strict — Never Fuzzy by Last Name Only',
      text:'EPISODE MEMORY (March 2026 — Payment matching incident). PROBLEM: Initial fuzzy payment matching used last-name similarity to map Zelle payments to CRM members. This caused 6 FALSE MATCHES: (1) Ranadhir Ghosh $375 was incorrectly matched to Dipra Ghosh, Rajanya Ghosh, Chandrani Ghosh, Suman Ghosh — all different families sharing "Ghosh" surname. (2) Papri Saha was falsely matched to Ishita Saha $145 — different families sharing "Saha" surname. (3) Barnali Mondal was falsely matched to Tarit Mondal $205 — different families sharing "Mondal" surname. ROOT CAUSE: Bengali community has many families with the same surname. Single-name fuzzy matching fails catastrophically. FIX: Use STRICT payment matching only: (a) Exact email match between Zelle payer and CRM member, (b) Exact full-name match (first + last), (c) Manual verification for edge cases. NEVER use single-token fuzzy matching. The corrected PAYMENT_MAP uses email-based matching verified against CSV ground truth. Previous falsely matched totalPaid values from 2025 (prior year) were also zeroed and preserved in totalPaid_prevYear. For 2026-27, only paymentStatus=paid with paymentAmount from Feb 27+ Zelle scan is valid.'});

    docs.push({id:'kb-episode-stale-totalpaid',year:'2026-27',category:'episode_memory',title:'Episode: Stale totalPaid Must Be Year-Specific',
      text:'EPISODE MEMORY (March 2026). PROBLEM: 72 CRM members had totalPaid values from 2025 (prior membership year). The QR food agent was using totalPaid > 0 to mark members as "PAID" — which was WRONG for the 2026-27 membership year that starts Feb 27, 2026. This made the Excel show many false "PAID" statuses. FIX: (1) Zeroed totalPaid for ALL 72 members (preserved original in totalPaid_prevYear). (2) Only paymentStatus=paid AND paymentAmount from Feb 27+ Zelle scan counts for 2026-27. (3) QR food agent was updated: paid field now checks (totalPaid > 0 || paymentAmount > 0 || paymentStatus === paid). MANDATORY PATTERN: totalPaid MUST be year-specific. When starting a new membership year, zero all totalPaid and archive to totalPaid_prevYear. Only payments after the membership year cutoff date count.'});

    docs.push({id:'kb-role-access-control',year:'2026-27',category:'security',title:'Portal Role-Based Access Control — EC vs Stakeholder Visibility',
      text:'BANF Portal Role Hierarchy (confirmed March 2026). ROLES: (1) super-admin — Full system access, only Ranadhir Ghosh. (2) admin — Senior EC Officers (Partha VP, Amit Treasurer, Rajanya GenSec): operational access, member dues, EC drive, but NO stakeholder panels. (3) ec-member — Coordinators (Moumita Cultural, Soumyajit Food, Sumanta Event, Rwiti Puja): EC collaboration, calendar, event reports, QR food. NO stakeholder panels, NO member dues. (4) business-stakeholder — Sponsors/community partners: stakeholder drive, feedback. CRITICAL RULE: The admin role must NOT see stakeholder panels (stakeholder-drive, feedback). Only super-admin and business-stakeholder roles can see stakeholder content. Ranadhir Ghosh is the ONLY EC member who is also a stakeholder (roles: super-admin + business-stakeholder + admin). All other EC members (including Rwiti) must NOT see stakeholder details.'});
