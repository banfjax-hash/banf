 ...
 *       d) If no perfect match ΓåÆ escalate to president
 ...
 *
 *  Architecture:
 *  ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ    ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ    ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ    ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ
 *  Γöé  Gmail    ΓöéΓöÇΓöÇΓöÇ>Γöé Bounce Parse ΓöéΓöÇΓöÇΓöÇ>Γöé CRM Lookup ΓöéΓöÇΓöÇΓöÇ>Γöé  Recovery   Γöé
 *  Γöé  Bounce   Γöé    Γöé (recipient,  Γöé    Γöé (member,   Γöé    Γöé  Strategy   Γöé
 *  Γöé  Emails   Γöé    Γöé  reason)     Γöé    Γöé  household)Γöé    Γöé  Execution  Γöé
 *  ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ    ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ    ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ    ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
 *       Γöé                                                       Γöé
 *       Γöé         ΓöîΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÉ       Γöé
 *       ΓööΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ>Γöé  State: delivery-failure-state.json Γöé<ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÿ
 *                 Γöé  - processedBounceIds               Γöé
 *                 Γöé  - flaggedEmails                     Γöé
 *                 Γöé  - recoveryActions                   Γöé
 *                 Γöé  - escalations                       Γöé
...
const CONFIG = {
  ...
  SPOUSE_CONFIDENCE_THRESHOLD: 0.95, // Only auto-recover if spouse match ΓëÑ 95%
...
// ΓöÇΓöÇ CLI Args (only active when run standalone) ΓöÇΓöÇ 
...
// ΓöÇΓöÇ HTTP Helper ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
 ...
 * 
 * Strategy priority:
 *   1. Alternate email on same CRM record     ΓåÆ recoverable (auto-redirect)
 *   2. Spouse with 100% match in household     ΓåÆ recoverable (verify + redirect)
 *   3. Other household member with email       ΓåÆ recoverable (verify + redirect)
 *   4. CRM record exists but no alt contacts   ΓåÆ escalate to president
 *   5. No CRM record at all                    ΓåÆ escalate to president
 */
function planRecoveryStrategy(bounce, crmResult, members) {
  ...
  if (bounce.isTemporary) {
    ...
    strategy.reason = `Temporary failure (${bounce.reasonCode}) ΓÇö will retry on next cycle`;
  ...
  if (!crmResult) {
    ...
    strategy.reason = `Email ${bounce.failedEmail} not found in CRM ΓÇö president review needed`;
  ...
  // 4. No recovery contacts ΓÇö escalate
  strategy.action = 'escalate';
  strategy.escalateToPresident = true;
...
function buildSpouseVerificationEmail(bounce, strategy) {
  ...
  return {
    subject: `BANF ΓÇö Email Delivery Issue for ${failedName}`,
    html: `<!DOCTYPE html>
<html>
...
<div class="container">
  <div class="header">
    <h1>≡ƒî╕ Bengali Association of North Florida</h1>
    <p>Important ΓÇö Email Delivery Issue</p>
  </div>
  <div class="body">
    <p>Dear ${spouseName},</p>

    <div class="alert-box">
      <strong>ΓÜá∩╕Å Email delivery failed</strong> for <strong>${failedName}</strong> (${bounce.failedEmail}).<br>
...
function buildPresidentEscalationEmail(bounce, strategy, member) {
  ...
  return {
    subject: `ΓÜá∩╕Å BANF Email Delivery Failure ΓÇö ${bounce.failedEmail} (Action Required)`,
    html: `<!DOCTYPE html>
<html>
<head><meta charset="UTF-8"><style>
  body { font-family: 'Segoe UI', Tahoma, sans-serif; margin: 0; padding: 0; background: #f5f5f5; }
...
<div class="container">
  <div class="header">
    <h1>ΓÜá∩╕Å Email Delivery Failure ΓÇö Escalation</h1>
  ...
  <div class="body">
    ...
    <div class="alert-box">
      ...
      <em>SMTP: ${bounce.smtpCode || 'N/A'} ΓÇö ${bounce.smtpDetail || 'N/A'}</em>
    </div>
...
async function sendRecoveryEmail(transporter, to, toName, emailContent) {
  if (DRY_RUN) {
    log('INFO', `  ≡ƒôº [DRY RUN] Would send to: ${toName} <${to}>`);
  ...
  try {
    ...
    log('INFO', `  Γ£à Sent ΓåÆ ${toName} <${to}> (${info.messageId})`);
    return { status: 'sent', messageId: info.messageId };
  } catch (e) {
    log('ERROR', `  Γ¥î FAILED ΓåÆ ${toName} <${to}>: ${e.message}`);
...
/**
 * Main scanning function ΓÇö can be called standalone or from bosonto-email-reader-agent.js
...
async function scanDeliveryFailures(existingState, existingToken) {
  ...
  log('INFO', 'ΓòÉΓòÉΓòÉ Delivery Failure Recovery Agent ΓÇö Scan Start ΓòÉΓòÉΓòÉ');
  ...
  for (const id of newIds) {
    try {
      ...
      if (bounce.failedEmail) {
        bounces.push(bounce);
        log('INFO', `  ≡ƒô¢ Bounce: ${bounce.failedEmail} ΓÇö ${bounce.reasonCode} (${bounce.isTemporary ? 'temp' : 'perm'})`);
      } else {
  ...
  // De-duplicate: if the same email already had action taken, just bump bounce count ΓÇö do NOT re-send
  ...
  for (const bounce of bounces) {
    ...
    if (existingFlag && existingFlag.actionTaken && existingFlag.suspended) {
      // Already handled ΓÇö just update bounce count & timestamp, skip recovery
      existingFlag.bounceCount = (existingFlag.bounceCount || 0) + 1;
      existingFlag.lastBounce = new Date().toISOString();
      log('INFO', `  ΓÅ╕∩╕Å  ${bounce.failedEmail} ΓÇö already actioned & suspended (bounce #${existingFlag.bounceCount}), skipping re-send`);
    ...
    if (strategy.targetEmail) {
      log('INFO', `    ΓåÆ Target: ${strategy.targetName} <${strategy.targetEmail}>`);
    }
  }
  ...
  for (const plan of recoveryPlans) {
    ...
    switch (strategy.action) {
      ...
      case 'redirect_alternate': {
        // Log the redirect ΓÇö actual resend would be separate
        log('INFO', `  ≡ƒöä Would redirect to alternate: ${strategy.targetEmail}`);
        actionRecord.targetEmail = strategy.targetEmail;
        actionRecord.note = 'Alternate email found ΓÇö can retry send to this address';
        recovered++;
        break;
      }
      ...
      case 'retry_later': {
        log('INFO', `  ΓÅ│ Will retry later: ${bounce.failedEmail} (${bounce.reasonCode})`);
        actionRecord.note = 'Temporary failure ΓÇö will be retried';
    ...
    // Flag the email address and mark action as taken ΓåÆ suspended until reactivated
    const existingFlag = state.flaggedEmails[bounce.failedEmail] || {};
  ...
  log('INFO', 'ΓòÉΓòÉΓòÉ Delivery Failure Recovery ΓÇö Complete ΓòÉΓòÉΓòÉ');
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
// isEmailFlagged ΓÇö Public API for other agents
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
function reactivateEmail(email) {
  try {
    ...
    saveState(state);
    log('INFO', `Γ£à Reactivated ${emailLower} ΓÇö was suspended since ${flag.suspendedAt} with ${previousBounces} bounces`);
...
<div class="header">
  <h1>≡ƒô¢ BANF Delivery Failure Recovery Report</h1>
...
<div class="summary">
  ...
  for (const [email, flag] of Object.entries(state.flaggedEmails)) {
    ...
    <div class="bounce-row">
      ...
      <div class="bounce-action ${actionClass}">
        Action: ${flag.recoveryAction || 'none'} ${flag.recoveryTarget ? `ΓåÆ ${flag.recoveryTarget}` : ''}
      </div>
    </div>`;
  }
  ...
  html += `
<div class="section">
  <div class="section-header">≡ƒô¢ Flagged Non-Deliverable Addresses (${Object.keys(state.flaggedEmails).length})</div>
  ${flaggedRows || '<div class="bounce-row" style="color: #66bb6a;">No flagged addresses ΓÇö all clear!</div>'}
</div>`;
  ...
  if (state.spouseVerifications.length > 0) {
    ...
    for (const v of state.spouseVerifications) {
      verRows += `
      <div class="bounce-row">
        <div class="bounce-email">${v.failedEmail} ΓåÆ ${v.spouseEmail}</div>
        <div class="bounce-reason">
    ...
    html += `
<div class="section">
  <div class="section-header">≡ƒÆæ Spouse Verification Emails Sent (${state.spouseVerifications.length})</div>
  ${verRows}
</div>`;
  }
  ...
  if (state.escalations.length > 0) {
    ...
    html += `
<div class="section">
  <div class="section-header">≡ƒö║ Escalations to President (${state.escalations.length})</div>
  ${escRows}
</div>`;
  }
...
async function main() {
  ...
  console.log('  ≡ƒô¢ BANF Delivery Failure Recovery Agent v1.0');
  console.log(`  ≡ƒôà ${new Date().toISOString()}`);
  console.log(`  ≡ƒöº Mode: ${DO_SEND ? 'LIVE SEND' : DRY_RUN ? 'DRY RUN' : REPORT_ONLY ? 'REPORT ONLY' : 'ANALYSIS'}`);
  console.log('ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ');
  ...
  if (REACTIVATE_EMAIL) {
    ...
    if (result.reactivated) {
      console.log(`  Γ£à ${email} reactivated (was ${result.previousBounces} bounces)`);
      console.log('  Future sends to this address are now allowed.');
    } else {
      console.log(`  Γ¥î Could not reactivate: ${result.error}`);
    }
    return;
  }
  ...
  console.log(`  ≡ƒô¢ Bounces: ${results.bouncesFound}`);
  console.log(`  Γ£à Recovered: ${results.recovered}`);
  console.log(`  ≡ƒö║ Escalated: ${results.escalated}`);
  console.log(`  ≡ƒÜ⌐ Flagged: ${results.flagged}`);
  console.log('ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ');
}
