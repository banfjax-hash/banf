 ...
 *  BANF Bosonto Utsob 2026 ΓÇö Continuous Email Reader Agent
 * ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
 *
 *  Polls Gmail every hour (configurable) for:
 *    1. New Evite RSVP notifications ΓåÆ pipeline + optional auto-send
 *    2. New Zelle/payment confirmation emails ΓåÆ CRM update
 *
 *  Modes:
...
// Agent Memory ΓÇö Vector RAG for long-term learning
const memory = require('./agent-memory-rag.js');

// Communication Compliance ΓÇö header encoding, content validation, audit
const compliance = require('./communication-compliance.js');

// Payment Purpose Detection Engine ΓÇö intelligent payment classification
...
// Delivery Failure Recovery Agent ΓÇö bounce detection, spouse recovery, escalation
const { scanDeliveryFailures, isEmailFlagged } = require('./banf-delivery-failure-agent.js');

// Reinforcement Learning Feedback Engine ΓÇö action recommendation from feedback
const rl = require('./banf-rl-feedback-engine.js');

// Event Manager Agent ΓÇö YAML-driven drive lifecycle controller
let eventManager;
try { eventManager = require('./banf-event-manager.js'); } catch (e) { eventManager = null; }

// Message Queue ΓÇö file-backed FIFO with dedup, DLQ, retry
const mq = require('./banf-message-queue.js');

// Development Instruct Agent ΓÇö email-driven dev workflow via GitHub CLI + Claude Opus
...
// ΓöÇΓöÇ CLI Args ΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇΓöÇ
...
if (args.includes('--help') || args.includes('-h')) {
  ...
  console.log('  --interval=N    Poll interval in minutes (default: 5)');
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
// STATE MANAGEMENT ΓÇö tracks processed emails
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
// Track token health ΓÇö avoid hammering Google when token is expired
...
async function getGmailToken() {
  ...
  if (_tokenFailCount >= TOKEN_FAIL_BACKOFF_CYCLES) {
    ...
    if (hoursSinceLastFail < 1) {
      throw new Error('Gmail token: expired/revoked (backoff ΓÇö retry in ' + Math.ceil(60 - hoursSinceLastFail * 60) + ' min). Run: node gmail-oauth-refresh.js');
    }
  ...
  for (const [cid, csec] of credentials) {
    ...
    if (resp.data.access_token) {
      // Token works ΓÇö reset failure tracking
  ...
  // Token failed ΓÇö track and notify
  _tokenFailCount++;
...
async function scanNewEviteEmails(state, existingToken) {
  ...
  for (const q of queries) {
    try {
      ...
      log('WARN', `Query failed: ${q} ΓÇö ${e.message}`);
    }
  }
  ...
  for (const id of newIds) {
    try {
      ...
      if (rsvp.guestName && rsvp.rsvp) {
        newRsvps.push(rsvp);
        log('INFO', `  NEW RSVP: ${rsvp.guestName} ΓåÆ ${rsvp.rsvp.toUpperCase()} (${rsvp.adults}a/${rsvp.kids}k)`);
      }
...
const INSTRUCTION_COMMANDS = {
  ...
  'send payment followup': async (args, state) => {
    ...
    log('INFO', `  ≡ƒôº Instruction: Sending payment followup to ${emailMatch[1]} for $${amountMatch[1]}`);
  ...
  'send rsvp email': async (args, state) => {
    ...
    log('INFO', `  ≡ƒôº Instruction: Sending RSVP email to ${member.email}`);
  ...
  'run scan': async (args, state) => {
    log('INFO', '  ≡ƒôº Instruction: Triggering email scan...');
    ...
    for (const [cmdPattern, handler] of Object.entries(INSTRUCTION_COMMANDS)) {
      if (body.includes(cmdPattern.toLowerCase())) {
        ...
        try {
          ...
          // Log result
          if (result.success) {
            log('INFO', `    Γ£ô Success: ${JSON.stringify(result.result).substring(0, 200)}`);
          } else {
            log('WARN', `    Γ£ù Failed: ${result.error || 'Unknown error'}`);
          }
        } catch (e) {
          log('ERROR', `    Γ£ù Error: ${e.message}`);
          results.push({
            instruction: instr,
            command: cmdPattern,
            success: false,
  ...
  for (const pat of GITHUB_FAILURE_PATTERNS) {
    if (pat.test(subject) || pat.test(body?.substring(0, 3000) || '')) {
      ...
      return {
        ...
        reason: `GitHub Actions failure: ${repo} ΓÇö ${workflow || subject}`
      };
    }
  }
  ...
  for (const id of newIds) {
    try {
      ...
      if (classification.isFailure) {
        ...
        log('INFO', `  ≡ƒö┤ GitHub Failure: ${classification.repo} ΓÇö ${msg.subject.substring(0,80)}`);
      }
...
6. Provide a summary of findings and actions taken`;
      ...
      log('INFO', `  ≡ƒöº Routed to dev pipeline: ${failure.repo} failure`);
    } catch (e) {
  ...
  for (const id of newIds) {
    try {
      ...
      if (devInstruct.isInstructEmail(email)) {
        ...
        results.push(email);
        log('INFO', `  ≡ƒöº NEW INSTRUCT: ${email.subject} from ${email.from}`);
      } else if (devInstruct.isInstructReply(email)) {
        email._instructType = 'reply';
        results.push(email);
        log('INFO', `  ≡ƒöº INSTRUCT REPLY: ${email.subject}`);
      }
  ...
  try {
    ...
    if (listResp.data && listResp.data.entries) {
      ...
      if (unenriched.length > 0) {
        ...
        if (updates.length > 0) {
          const updResp = await httpsRequest(LEDGER_API + '/ledger_update', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ adminKey: LEDGER_ADMIN_KEY, updates })
          });
          if (updResp.data && updResp.data.success) {
            enriched = updResp.data.updated || 0;
            log('INFO', `  [ExpenseIQ] Enriched ${enriched} existing entries with event/category intelligence`);
          } else {
            log('WARN', `  [ExpenseIQ] Update failed: ${JSON.stringify(updResp.data).substring(0, 200)}`);
          }
        }
      }
    }
  } catch (e) {
    log('WARN', `  [ExpenseIQ] Re-enrichment error: ${e.message}`);
  }

  return { scanned: newIds.length, newEntries: entries.length, posted, enriched };
}
...
async function scanUserQueryEmails(state, token) {
  ...
  for (const id of newIds) {
    try {
      ...
      // ΓöÇΓöÇ FILTER 1: Marketing / Spam / Automated ΓöÇΓöÇ
      ...
      // ΓöÇΓöÇ FILTER 2: Already replied by BANF ΓöÇΓöÇ
      ...
      // ΓöÇΓöÇ DETECT: Member reply to BANF-sent email (like Subrata's) ΓöÇΓöÇ
...
async function processUserQueries(queries, state) {
  ...
  for (const query of queries) {
    try {
      // ΓöÇΓöÇ RL Pre-Filter: ask the RL engine if we should even process this ΓöÇΓöÇ
      ...
      if (rlRec.action === 'DO_NOTHING' && rlRec.confidence >= 0.80) {
        log('INFO', `    ≡ƒñû RL: ${query.subject.substring(0, 40)}... ΓåÆ DO_NOTHING (${(rlRec.confidence * 100).toFixed(0)}%) ΓÇö silent drop`);
        results.push({
          query,
      ...
      if (rlRec.action === 'NOTIFY_PRESIDENT' && rlRec.confidence >= 0.80) {
        log('INFO', `    ≡ƒñû RL: ${query.subject.substring(0, 40)}... ΓåÆ NOTIFY_PRESIDENT (${(rlRec.confidence * 100).toFixed(0)}%) ΓÇö flag only, no response`);
      ...
      if (result.requiresApproval) {
        log('INFO', `    ΓÅ│ ${query.subject.substring(0, 30)}... ΓåÆ PENDING APPROVAL (${result.classification.category})`);
      } else if (result.autoApproved) {
        log('INFO', `    Γ£à ${query.subject.substring(0, 30)}... ΓåÆ AUTO-APPROVED & SENT`);
      } else {
        log('INFO', `    ≡ƒôï ${query.subject.substring(0, 30)}... ΓåÆ QUEUED for processing`);
      }
    } catch (e) {
      log('ERROR', `  Γ¥î Failed to process query: ${e.message}`);
      results.push({
        query,
        error: e.message,
        processedAt: new Date().toISOString()
  ...
  for (const rsvp of newRsvps) {
    ...
    if (crmMatch) {
      ...
      if (isNew) {
        ...
        log('INFO', `  Γ£¿ NEW member in pipeline: ${memberData.displayName} <${memberData.email}> ΓåÆ ${rsvp.rsvp} (${tier})`);
      } else {
        updatedMembers.push(memberData);
        log('INFO', `  ≡ƒöä UPDATED: ${memberData.displayName} ΓåÆ ${rsvp.rsvp} (was already in pipeline)`);
      }
    } else {
      log('WARN', `  Γ¥ô UNMATCHED: ${rsvp.guestName} (${rsvp.guestEmail || 'no email'}) ΓåÆ ${rsvp.rsvp}`);
      state.newRsvps.push({
...
async function autoSendPaymentFollowups(newPayments, state) {
  ...
  for (const payment of newPayments) {
    ...
    if (!crmMatch) {
      log('WARN', `  Γ¥ô Payment $${payment.amount} from ${payment.payerName} - NO CRM MATCH (manual followup needed)`);
    ...
    // ΓöÇΓöÇ Payment Purpose Classification (v1.0) ΓöÇΓöÇ
    ...
    log('INFO', `  ≡ƒöì Purpose: ${paymentPurpose} (${purposeConf}% confidence) | ${generatePaymentAcknowledgment(purposeClassification)}`);

    // If membership already paid, flag as possible sponsorship/donation
    if (purposeClassification.membershipAlreadyPaid) {
      log('INFO', `  ΓÜá∩╕Å  Membership already paid for FY${getFiscalYear(new Date())} ΓåÆ payment may be ${paymentPurpose}`);
    }
    ...
    log('INFO', `  ≡ƒÆ░ Payment $${payment.amount} from ${m.displayName} ΓåÆ ${isPartial ? 'PARTIAL' : 'FULL'} (${tier})`);

    // Check if recipient email is flagged as non-deliverable
    const flagCheck = isEmailFlagged(m.email);
    if (flagCheck.flagged) {
      log('WARN', `  ΓÜá∩╕Å Email ${m.email} is flagged: ${flagCheck.reason} (bounces: ${flagCheck.bounceCount})`);
      if (flagCheck.recoveryTarget) {
        log('INFO', `    ΓåÆ Recovery target: ${flagCheck.recoveryTarget}`);
        memberData.email = flagCheck.recoveryTarget; // Redirect to recovery target
        log('INFO', `    ΓåÆ Redirecting payment followup to ${flagCheck.recoveryTarget}`);
      } else {
        log('WARN', `    ΓåÆ No recovery target ΓÇö skipping send`);
    ...
    try {
      ...
      if (resp.status === 200 && resp.data.success) {
        log('INFO', `    Γ£ë∩╕Å Payment followup sent to ${m.email}`);
        ...
        log('ERROR', `    Γ¥î Failed to send payment followup to ${m.email}: ${JSON.stringify(resp.data)}`);
        results.push({ payment, member: m.email, status: 'failed', error: resp.data });
      }
    } catch (e) {
      log('ERROR', `    Γ¥î Payment followup error for ${m.email}: ${e.message}`);
...
async function autoSendNewMembers(newMembers, state) {
  ...
  for (const m of newMembers) {
    ...
    if (flagCheck.flagged && !flagCheck.isTemporary) {
      log('WARN', `  ΓÜá∩╕Å Skipping ${m.email} ΓÇö flagged: ${flagCheck.reason}`);
      if (flagCheck.recoveryTarget) {
        log('INFO', `    ΓåÆ Would redirect to ${flagCheck.recoveryTarget} (manual action needed)`);
      }
      continue;
    }
    ...
    try {
      ...
      if (resp.status === 200) {
        ...
        for (const r of results) {
          ...
          log('INFO', `    Γ£ë∩╕Å  ${r.displayName}: ${sentCount} sent (${emailType})`);
          state.sentEmails.push({
            email: r.email,
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
// PIPELINE STATUS ΓÇö writes JSON for the EC admin portal
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
...
function writePipelineStatus(state, cycleResult, elapsedSec) {
  ...
  // Active drives detection ΓÇö use Event Manager if available, else fallback to heuristic
  let activeDrives = [];
  if (eventManager) {
    try {
  ...
  // Recent activity ΓÇö last 10 events
  const recentActivity = [];
  const addRecent = (items, type) => {
    (items || []).slice(-5).forEach(item => {
      recentActivity.push({
        type,
        name: item.displayName || item.payerName || item.email || 'ΓÇö',
        email: item.email || item.payerEmail || 'ΓÇö',
        time: item.detectedAt || item.sentAt || now.toISOString(),
        detail: type === 'payment' ? `$${item.amount}` : (item.rsvp || item.status || 'ΓÇö'),
  ...
  const status = {
    ...
    driveConfig: eventManager && typeof eventManager.getActiveDrives === 'function' ? {
  ...
  try {
    ...
    log('INFO', `  Pipeline status written ΓåÆ ${PIPELINE_STATUS_FILE}`);
  } catch (e) {
...
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
// MESSAGE QUEUE INTEGRATION ΓÇö enqueue, process, ack/nack
// ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ
 ...
 * Failed items get nack'd ΓåÆ retry or DLQ.
 */
function ackProcessedMessages(phase, items, results) {
  ...
  // Drain the queue ΓÇö ack all since they've been processed by the scan functions
  let acked = 0;
  let msg;
  ...
  if (acked > 0) {
    log('INFO', `  ≡ƒô¼ MQ: ${phase} ΓÇö ${acked} messages acknowledged`);
  }
}
...
async function runOnce(state) {
  ...
  log('INFO', `ΓòÉΓòÉΓòÉ Poll cycle #${state.pollCount} ΓòÉΓòÉΓòÉ`);

  // ΓöÇΓöÇ Sync drive states from YAML config ΓöÇΓöÇ
  if (eventManager) {
    try {
      eventManager.syncDriveState();
      const active = eventManager.getActiveDrives();
      log('INFO', `  ≡ƒÄ¢∩╕Å  Active drives: ${active.map(d => d.driveId).join(', ') || 'none'}`);
    } catch (e) {
      log('WARN', `Drive sync failed (continuing without drive gating): ${e.message}`);
    }
  }

  // Phase 1: Scan for new Evite RSVPs (EVENT_BOUND ΓÇö gated by drive state)
  let newRsvps = [], token = null;
  const eviteActive = !eventManager || typeof eventManager.isDriveActive !== 'function' || eventManager.isDriveActive('evite_rsvp');

  // Acquire Gmail token first ΓÇö if this fails, log error and skip all Gmail phases
  try {
    ...
    // Still save state and return ΓÇö don't crash the poll loop
    saveState(state);
  ...
  if (eviteActive) {
    ...
    log('INFO', `  ΓÅ╕∩╕Å  Evite RSVP drive is DORMANT ΓÇö skipping Phase 1`);
  }

  // Phase 2: Scan for new payments (ALWAYS_ON)
  let newPayments = [];
  try {
    newPayments = await scanNewPaymentEmails(state, token);
    if (newPayments.length > 0) enqueueScannedEmails('payment', newPayments);
  } catch (e) {
    log('WARN', `Payment scan failed (continuing): ${e.message}`);
  }
  ...
  try {
    newInstructions = await scanInstructionEmails(state, token);
    if (newInstructions.length > 0) enqueueScannedEmails('admin_instruction', newInstructions);
    instructionResults = await processInstructions(newInstructions, state);
    ackProcessedMessages('admin_instruction', newInstructions, instructionResults);
  } catch (e) {
    log('WARN', `Instruction scan failed (continuing): ${e.message}`);
  }

  // Phase 2b-instruct: Development Instruct Agent (ALWAYS_ON)
  // Detects emails with exact subject "INSTRUCT" from president ΓåÆ routes to dev instruct agent
  ...
  try {
    ...
    for (const email of allInstructs) {
      try {
        ...
        log('INFO', `  ≡ƒöº Dev Instruct: processed new INSTRUCT email ΓÇö ${email.subject}`);
      } catch (e) {
    ...
    for (const reply of replyEmails) {
      try {
        ...
        log('INFO', `  ≡ƒöº Dev Instruct: processed reply ΓÇö ${reply.subject}`);
      } catch (e) {
  ...
  let queryResults = [];
  try {
    userQueries = await scanUserQueryEmails(state, token);
    if (userQueries.length > 0) enqueueScannedEmails('user_query', userQueries);
    queryResults = await processUserQueries(userQueries, state);
    ackProcessedMessages('user_query', userQueries, queryResults);
  } catch (e) {
    log('WARN', `User query scan failed (continuing): ${e.message}`);
  }
  ...
  try {
    ...
    if (deliveryFailureResults.bouncesFound > 0) {
      log('INFO', `  ≡ƒô¢ Delivery failures: ${deliveryFailureResults.bouncesFound} bounces, ${deliveryFailureResults.recovered} recovered, ${deliveryFailureResults.escalated} escalated`);
    }
  } catch (e) {
  ...
  try {
    ...
    if (ghFailures.length > 0) {
      ...
      log('INFO', `  ≡ƒö┤ GitHub failures: ${ghFailures.length} detected, ${githubFailureResults.routed} routed to dev pipeline`);
    }
  } catch (e) {
  ...
  // Summary
  log('INFO', `ΓöÇΓöÇΓöÇ Cycle #${state.pollCount} complete (${elapsed}s) ΓöÇΓöÇΓöÇ`);
  ...
  if (devInstructResults.newInstructions > 0 || devInstructResults.replies > 0) {
    log('INFO', `  ≡ƒöº Dev Instruct: ${devInstructResults.newInstructions} new, ${devInstructResults.replies} replies`);
  }
  ...
  if (githubFailureResults.detected > 0) {
    log('INFO', `  ≡ƒö┤ GitHub Actions failures: ${githubFailureResults.detected} detected, ${githubFailureResults.routed} routed to dev pipeline`);
  }
  ...
  // ΓöÇΓöÇ MQ: Ack RSVPs after processing ΓöÇΓöÇ
  ackProcessedMessages('evite_rsvp', newRsvps, []);

  // ΓöÇΓöÇ MQ: Status summary ΓöÇΓöÇ
  ...
  if (mqPending > 0 || mqDLQ > 0) {
    log('INFO', `  ≡ƒô¼ MQ: ${mqPending} pending, ${mqDLQ} in DLQ`);
  }

  // ΓöÇΓöÇ Write pipeline status JSON for EC admin portal ΓöÇΓöÇ
...
async function main() {
  ...
  console.log('  ≡ƒî╕ BANF Bosonto Utsob 2026 ΓÇö Continuous Email Reader Agent');
  console.log(`  ≡ƒôí Mode: ${DO_POLL ? `CONTINUOUS (every ${POLL_INTERVAL_MIN} min)` : 'SINGLE RUN'}`);
  console.log(`  ≡ƒôº Auto-send: ${AUTO_SEND ? 'YES' : 'NO'}`);
  console.log(`  ≡ƒôª Batch size: ${BATCH_SIZE}`);
  console.log('ΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉΓòÉ');
  ...
  if (!DO_POLL) {
    // Single run
    await runOnce(state);
    console.log('\nΓ£à Done. Use --poll for continuous monitoring.');
    return;
  }
  ...
  // Graceful shutdown
  process.on('SIGINT', () => {
    log('INFO', 'Received SIGINT ΓÇö shutting down gracefully...');
    clearInterval(timer);
    saveState(state);
  ...
  process.on('SIGTERM', () => {
    log('INFO', 'Received SIGTERM ΓÇö shutting down...');
    clearInterval(timer);
    saveState(state);
    process.exit(0);
  });
